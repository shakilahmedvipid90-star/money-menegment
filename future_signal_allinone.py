...
- Dynamic Trend-Momentum Scoring Engine (Zero Random Fallback)
- Rejection Wick & False Breakout Protection
- 10-Minute Loss Cooldown Shield
- 1-3 Minutes Smart Scanning Interval
- Stealth VIP Schedule Mode & Multi-Broker APIs (Quotex, Pocket Option, Forex)
"""
...
# ================= WEIGHTED HIGH-ACCURACY ANALYSIS ENGINE =================
...
def analyze_best_pair_and_trend(pair_pool, broker_type="quotex", chat_id=None):
    ...
    for p in pair_pool:
        ...
        if closes[-2] < opens[-2]:
            put_score += 15.0   
        if lower_wick < (candle_body * 0.4):
            put_score += 20.0   
        if 32 <= rsi_val <= 52:
            put_score += 15.0   

        if call_score > put_score and call_score >= 50:
            scored_candidates.append((call_score, p, "CALL", "High Probability Trend Flow"))
        elif put_score > call_score and put_score >= 50:
            scored_candidates.append((put_score, p, "PUT", "High Probability Breakdown Flow"))
    ...
    with batch_disk_lock:
        ...
        for c_id, b in active_batches.items():
            ...
            serializable[c_id] = {
                ...
                "signals": signals_clean(sigs_copy)
            }
        save_json(ACTIVE_BATCHES_FILE, serializable)

def signals_clean(sigs):
    for s in sigs:
        if isinstance(s.get("target_dt"), datetime):
            s["target_dt"] = s["target_dt"].isoformat()
    return sigs
...
def build_scanning_card():
    return (
        ...
        " ⚡ <b>Scanning:</b> <i>All pairs for high probability flow...</i>\n"
        " ⏳ <i>Analyzing momentum...</i>\n"
        "───────────────✦───────────────"
    )
...
def auto_mode_loop(chat_id, username=None, broker_type="quotex"):
    ...
    while auto_mode_users.get(c_id, False):
        ...
        scan_delay_seconds = random.randint(70, 150)
        elapsed_scan = 0
...
def scheduled_channel_session_worker(admin_chat_id, target_channel, start_dt, end_dt, alert_dt, broker_type="quotex"):
    ...
    session_start_msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🌐 <b>Market:</b> <code>{m_label}</code>\n"
        f"🎯 <b>Setups:</b> <code>Dynamic Trend Momentum Flow</code>\n"
    ...
    while datetime.now(user_tz) < end_dt and session_info["is_running"]:
        ...
        scan_delay_seconds = random.randint(90, 180)
        elapsed_scan = 0
...
def run_server():
    ...
    def send_main_menu(chat_id, username="", target_msg_id=None):
        ...
        text = (
            ...
            "╰──────────────────────╯\n\n"
            "⚡️ <b>CORE ENGINE:</b> Strict Trend Momentum Math 🤖\n"
            "📈 <b>SPEED:</b> Real-Time 100% Broker Match ⚡️\n"
            "🚀 <b>ALGORITHM:</b> Weighted Momentum & Wick Health Filter 🧠\n"
            "🛡 <b>RISK CONTROL:</b> 10-Min Loss Cooldown & Martingale Protection 🔒\n"
            ...
            "🎯 Zero Random Fallbacks (Pure Math Signals)\n"
            "🛡 Dynamic Reversal Shielding\n"
